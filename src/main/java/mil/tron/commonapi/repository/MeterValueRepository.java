...
public interface MeterValueRepository extends CrudRepository<MeterValue, UUID>{
    ...
    @Query("SELECT c.appEndpoint.id AS id, c.appEndpoint.path AS name, SUM(c.value) AS sum "
        + "FROM MeterValue AS c WHERE c.appSource.id = ?1 AND c.timestamp >= ?2 AND c.timestamp <= ?3 "
        + "GROUP BY c.appEndpoint.id, c.appEndpoint.path ORDER BY c.appEndpoint.path ASC")
    List<CountMetric> sumByEndpoint(UUID appSource, Date startDate, Date endDate);
    ...
    @Query("SELECT c.appEndpoint.id AS id, c.appEndpoint.path AS name, SUM(c.value) AS sum "
        ...
        + "GROUP BY c.appEndpoint.id, c.appEndpoint.path ORDER BY c.appEndpoint.path ASC")
    List<CountMetric> sumByAppSourceAndEndpointForAppClient(UUID appSource, String name, Date startDate, Date endDate);
}
