...
public class ScratchStorageControllerTest {
    ...
    @Test
    void testAddingUserPrivToAppPlainText() throws Exception {
        ...
        mockMvc.perform(post(SCRATCH_ENDPOINT + "{appId}/addUser", UUID.randomUUID())
    ...
    @Test
    void testSetImplicitReadByAppIdPlainText() throws Exception {
        // test we can access the implicit read setter route
        UUID appId = entries.get(0).getAppId();
        Mockito.when(service.setImplicitReadForApp(Mockito.any(UUID.class), Mockito.anyBoolean()))
                .thenReturn(registeredApps.get(0));
        ...
        mockMvc.perform(post(ENDPOINT  +"apps/{appId}/implicitRead?value=true", appId)
                .contentType(MediaType.TEXT_PLAIN))
                .andExpect(status().isOk());
    }

    @Test
    void testCreateUpdateKeyValuePairPlainText() throws Exception {
        // test we can add/update at key value pair (just like a real hash would be)
    ...
    @Test
    void testDeleteAllByAppIdPlainText() throws Exception {
        // test we can delete all key value pairs for an app given its ID
        ...
        mockMvc.perform(post(ENDPOINT + "deleteAllKeys/{appId}", entries.get(0).getAppId())
                .contentType(MediaType.TEXT_PLAIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].appId", equalTo(entries.get(0).getAppId().toString())));
    }
    ...
    @Test
    void testDeleteAllByAppIdAndKeyNamePlainText() throws Exception {
        // test we can delete a specific key value pair for an app given its ID
        ...
        mockMvc.perform(post(ENDPOINT + "deleteKey/{appId}/key/{keyName}", entries.get(0).getAppId(), entries.get(0).getKey())
                .contentType((MediaType.TEXT_PLAIN)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.appId", equalTo(entries.get(0).getAppId().toString())));
    ...
    @Test
    void testRemovingUserPrivFromAppPlainText() throws Exception {
        ...
        mockMvc.perform(post(SCRATCH_ENDPOINT + "{appId}/removeUser/{privId}", UUID.randomUUID(), UUID.randomUUID())
                .contentType(MediaType.TEXT_PLAIN)
                .content(OBJECT_MAPPER.writeValueAsString(priv)))
                .andExpect(status().isForbidden());
